...
class ImprovedFaceDetector:
    def __init__(self):
        ...
        if os.path.exists(JSON_FILE):
            try:
                ...
                self.authorized_image_paths = [path for path in loaded_paths if os.path.exists(path) and os.path.isfile(path)]
                with open(JSON_FILE, 'w') as f:
                    json.dump(self.authorized_image_paths, f)
            except (json.JSONDecodeError, Exception) as e:
                st.sidebar.warning(f"Error reading JSON file: {str(e)}. Resetting to empty list.")
                with open(JSON_FILE, 'w') as f:
                    json.dump([], f)
    ...
    def load_images(self):
        ...
        for idx, image_path in enumerate(self.authorized_image_paths):
            if not os.path.exists(image_path) or not os.path.isfile(image_path):
                st.sidebar.warning(f"Image file not found or invalid: {image_path}")
                continue
            try:
                ...
                if faces is not None and len(faces) > 0:
                    ...
                    st.sidebar.warning(f"⚠️ No face found in authorized image: {image_path}")
            except Exception as e:
    ...
    def remove_images(self):
        """Remove all authorized images and delete files listed in JSON"""
        if os.path.exists(JSON_FILE):
            try:
                with open(JSON_FILE, 'r') as f:
                ...
                for file_path in image_paths:
                    if os.path.exists(file_path) and os.path.isfile(file_path):
                        ...
                        st.sidebar.warning(f"⚠️ File not found: {os.path.basename(file_path)}")
    ...
    def process_frame(self, frame, threshold=0.75):
        ...
        try:
            ...
            st.warning(f"⚠️ Frame processing error: {str(e)}")
            return frame, []
...
def main():
    ...
    st.write(f"Debug: Authorized directory: {os.path.abspath(AUTHORIZED_DIR)}")
    ...
    with st.sidebar:
        ...
        if st.button("Load Authorized Images") and uploaded_files:
            ...
            if new_paths:
                st.session_state.detector.authorized_image_paths = new_paths
                with open(JSON_FILE, 'w') as f:
        ...
        if st.session_state.detector.authorized_image_paths:
            ...
            for idx, img_path in enumerate(st.session_state.detector.authorized_image_paths):
                st.write(f"Debug: Attempting to open image: {img_path}")
                if not os.path.exists(img_path) or not os.path.isfile(img_path):
                    st.warning(f"⚠️ Image file not found or invalid: {img_path}")
                    continue
                try:
                    with cols[idx % 3]:
    ...
    with col1:
        ...
        if st.session_state.webcam_active:
            try:
                ...
                st.error(f"❌ WebRTC error: {str(e)}\nEnsure browser supports WebRTC and camera permissions are granted")
                st.session_state.webcam_active = False
