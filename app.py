...
def main():
    ...
    if st.sidebar.button("Load Authorized Images"):
        if authorized_uploads:
            ...
            st.sidebar.success("Images loaded successfully!")
        else:
    ...
    if st.sidebar.button("Remove Authorized Images"):
        ...
        st.sidebar.success("All images removed!")
    
    # Display authorized images in sidebar
    if st.session_state.detector.authorized_image_paths:
        st.sidebar.subheader("Authorized Persons")
        # Create columns for image display
        num_cols = min(3, len(st.session_state.detector.authorized_image_paths))
        ...
        for idx, img_path in enumerate(st.session_state.detector.authorized_image_paths):
            try:
                with cols[idx % num_cols]:
                    ...
                    st.image(img, caption=f"Person {idx + 1}", use_column_width=True)
            except Exception as e:
                st.sidebar.error(f"Error displaying image {idx + 1}: {str(e)}")
